...
public class ProcessData {
     ...
     * a different order).
     * @param visualizeLetters
     * @param guesses2
     * @return All the possible words the user may thinking of.
     */
    public static String letsGuess(String possibleWord, int m[], int[] guesses, String visualizeLetters[],int guesses2) {
        int v[] = new int[26];
...
//        visualizeWord(guesses[0], visualizeLetters);
        boolean maybe = guessLetter(asciiLetter);

        if (maybe == true) {
            guesses[0]--;
            whatPositions(visualizeLetters, letter);
//            int contador = 0;
//            while (visualizeLetters[contador] != null) {
//                contador++;
//            }
//            visualizeLetters[contador] = letter;

            visualizeWord(guesses2, visualizeLetters);
     ...
     * @param guesses
     */
    private static void visualizeWord(int guesses, String[] visualizeLetters) {
        boolean flag = false;
        int guesses_copy = guesses;
        for (int i = 0; i < guesses_copy; i++) {
            for (int j = 0; j < guesses_copy; j++) {
                if (visualizeLetters[j] != null) {
                    String v[] = visualizeLetters[j].split("_");
//                    System.out.println(v[1]+" eh " + guesses_copy);
//                    System.out.println(visualizeLetters[j]);
//                    System.out.println("ola "+v[j]);
                    for (int z = 1; z < v.length-1; z++) {
                        if (Integer.parseInt(v[z]) == i) {
//                            System.out.println(Integer.parseInt(v[z])+"=== i");
...
//                    break;
    ...
    private static void whatPositions(String[] visualizeLetters, String letter) {
        int contador = 0;
        while (visualizeLetters[contador] != null) {
            contador++;
        }
        ...
        System.out.println("What positions of the word does the letter appear? (insert -1 when they're all filled)");

        while (positions != -1) {
            try {
                positions = in.nextInt();
                visualizeLetters[contador] += "_" + positions;
            } catch (Exception e) {
                System.out.println("Insert a number.");
